    ...
    try {
      ...
      if (result.meta.requestStatus === 'fulfilled') {
        dispatch(addNotification({
          type: 'success',
          message: 'Account created successfully! Welcome to SplitSync.',
        }));
      }
    } catch (err) {
      dispatch(addNotification({
        ...
        <div className="bg-white/10 backdrop-blur-lg p-8 rounded-2xl shadow-xl border border-white/20">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <button
              type="submit"
              disabled={isLoading}
              className={`w-full font-medium py-3 px-4 rounded-xl transition-all duration-300 ${
                isLoading
                  ? 'bg-gray-600 cursor-not-allowed'
                  : 'bg-gradient-to-r from-indigo-500 to-cyan-500 text-white hover:scale-105 hover:shadow-lg hover:animate-pulse'
              }`}
            >
              {isLoading ? (
